...
dat <- load("C:/Users/ETay/Documents/Booster doses/Data/booster_sd_sample_20241112.Rda")
...
#averaging over conds and concoms
for(s in 1:2){
  for(i in 1:2){
    for(k in 1:3){
      for(a in 1:N_age){
...
pred_conds <- array(dim = c(nrow(postr), S, K, N_age,length(conds)),
              dimnames = list(Draw = 1:nrow(postr), Sex = 1:S, Vax = 1:K, Age = 1:N_age, Conds = conds))



#pred input for conds
for(s in 1:2){
  for(k in 1:3){
      for(a in 1:N_age){
        if(sum(sex == s & vax == k & x == ages[a]) > 0){
          tmp <- matrix(W[sex == s & vax == k & x == ages[a],], ncol = length(conds))
          prop <- colMeans(tmp)
        } else {
          prop <- rep(1/length(conds), length(conds))
        }
        pred_conds[,s,k,a] <- postr[,"alpha"] +
                          postr[,paste0("f[", s, ",", i, ",", k, ",", a, "]")] +
                          as.vector(prop%*%t(postr[,paste0("beta[", 1:length(conds), "]")]))
      }
  }
...
pred_vis[, `:=`(
  ...
  Vax = factor(Vax, labels = c("Pf", "Mod", "Nvx")),
  Sex = factor(Sex, labels = c("Female", "Male")),
  Conds = factor(Conds, labels = c("Heart disease",
                                   "Poorly controlled blood pressure", "Diabetes", "Chronic lung disease",
                                   paste0("Obesity (BMI ", "\U2265", "40)"), "Chronic kidney failure", "Chronic liver disease",
                                   "Cancer (Solid organ)", "Haematologial cancer", "Receiving chemotherapy or radiotherapy",
                                   "Organ transplant recipient", "Bone marrow transplant recipient", "Neurological condition",
...
#plot vis_list
  ...
  ### Figure 2 (MA for medical conditions)
